...
std::vector<Object> GetDetectionResults(const tflite::Interpreter& interpreter,
  ...
  std::vector<const std::string*> signature_keys = interpreter.signature_keys();
  if (!signature_keys.empty()) {
    CHECK_EQ(signature_keys.size(), 1);
    VLOG(1) << "Signature name: " << *signature_keys[0];
    const auto& signature_output_map = interpreter.signature_outputs(
        signature_keys[0]->c_str());
