...
TrainingData ShuffleAndSplitData(const MatrixXf& data_matrix,
  ...
  shuffled_data = data_matrix(shuffled_indices, Eigen::indexing::indexing::all);
  ...
  fake_data.training_data =
      shuffled_data(Eigen::seq(0, num_train - 1), Eigen::indexing::indexing::all);
  fake_data.validation_data =
      shuffled_data(Eigen::seq(num_train, Eigen::last), Eigen::indexing::indexing::all);
